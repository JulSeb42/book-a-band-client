...
export const COMMON_TEXTS = {
	...
	EMAIL_SIGNUP_BODY: (user: User, token: string) =>
		`Hello ${user.fullName},<br /><br />Thank you for creating your account on ${SITE_DATA.NAME}! <a href="${API_URL}/verify?token=${token}&id=${user._id}">Click here to verify your account.</a>.`,

	EMAIL_RESET_PASSWORD_TITLE: `Reset your password on ${SITE_DATA.NAME}`,
	EMAIL_RESET_PASSWORD_BODY: (user: User, token: string) =>
		`Hello ${user.fullName},<br /><br />To reset your password, <a href="${API_URL}/reset-password/${token}/${user._id}">click here</a>.`,

	USER_DELETED: "User deleted.",

	SEARCH_NO_RESULT: "Your search did not return any result.",
